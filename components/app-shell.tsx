...
export function AppShell({
  ...
  const visibleNavItems = navItems.filter((item) =>
    hasCapability(currentUser?.role, item.capability),
  );
  ...
  const profileName = header.profileName || currentUser?.fullName || "User";
  ...
  return (
    <div className="agrotrust-admin-surface min-h-screen overflow-x-hidden bg-[#f1f1f1] text-[#2b2b2f] lg:pl-[170px]">
      <Sidebar activeNav={activeNav} items={visibleNavItems} />
      <MobileNavigation
        activeNav={activeNav}
      ...
      <div className="min-w-0">
        <header className="z-20 border-b border-[#eeeeee] bg-white lg:sticky lg:top-0">
          <div className="flex min-h-[62px] w-full flex-wrap items-center justify-between gap-x-4 gap-y-3 px-4 py-3 sm:flex-nowrap sm:px-6 sm:py-0 lg:px-5">
            <div className="min-w-0">
              <h1 className="text-[18px] font-semibold leading-[23px] text-[#2b2b2f]">
                {header.title}
              </h1>
              <p className="text-[12px] font-medium leading-[16px] text-[#a7a7aa]">
                {header.dateLabel}
              </p>
            </div>
            <div
            ...
            >
              ...
              >
                ...
                {unreadCount > 0 ? (
                  <span className="absolute right-[7px] top-[6px] h-[6px] w-[6px] rounded-full bg-[#ef4f55]" />
                ) : null}
              </motion.button>
              <motion.button
                aria-expanded={openMenu === "profile"}
                aria-label={
                  profileName ? `${profileName} profile` : header.avatarLabel
                }
                ...
                whileTap={{ scale: 0.97 }}
              >
                <InitialAvatar name={profileName} size="small" />
                <ChevronDown
                  className={`hidden h-4 w-4 text-[#77777d] transition sm:block ${openMenu === "profile" ? "rotate-180" : ""}`}
                />
              </motion.button>
              <HeaderPopovers
        ...
        <motion.main
          ...
          className={`grid w-full ${mainClassName}`}
        >
          {children}
        </motion.main>
      </div>
      ...
      {openMenu === "notifications" ? (
        ...
        >
          <div className="flex items-start justify-between gap-4 border-b border-[#eeeeef] px-4 py-3">
            <div>
              <h2 className="text-[14px] font-semibold leading-[20px] text-[#2f2f34]">
                Notifications
              </h2>
              <p className="text-[12px] font-medium leading-[18px] text-[#99999f]">
                Latest account updates
              </p>
            </div>
            <a
              className="mt-0.5 inline-flex shrink-0 items-center gap-1 text-[12px] font-semibold leading-[18px] text-[#2d5f9f] transition hover:text-[#164780]"
          ...
          <div className="max-h-[330px] overflow-y-auto p-2">
            {notifications.length > 0 ? (
              notifications.map((notification) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <span
                      className={`mt-0.5 h-8 w-8 shrink-0 rounded-full ${notificationToneStyles[notification.tone]}`}
                    >
                    ...
                    <div className="min-w-0">
                      <div className="flex items-start gap-2">
                        <h3 className="text-[12px] font-semibold leading-[17px] text-[#333338] group-hover:text-[#15447C]">
                          {notification.title}
                        </h3>
                        {notification.unread ? (
                          <span className="mt-1.5 h-[6px] w-[6px] shrink-0 rounded-full bg-[#ef4f55]" />
                        ) : null}
                      </div>
                      <p className="mt-1 text-[11px] font-medium leading-[16px] text-[#87878d]">
                        {notification.description}
                      </p>
                      <p className="mt-1 text-[11px] font-medium leading-[16px] text-[#aaaab0]">
              ...
              <div className="px-3 py-8 text-center text-[13px] font-medium leading-[20px] text-[#8d8d93]">
                No notifications
              </div>
            )}
          </div>
          <div className="border-t border-[#eeeeef] p-2">
            <a
              className="flex h-10 items-center justify-center gap-2 rounded-[6px] text-[12px] font-semibold leading-[18px] text-[#2d5f9f] transition hover:bg-[#f7faff] hover:text-[#164780]"
              href="/transactions?tab=alerts"
            >
      ...
      {openMenu === "profile" ? (
        ...
        >
          <div className="flex items-center gap-3 border-b border-[#eeeeef] px-4 py-3">
            <InitialAvatar name={profileName} size="large" />
            <div className="min-w-0">
              <p className="truncate text-[13px] font-semibold leading-[18px] text-[#303034]">
                {profileName}
              </p>
              <p className="truncate text-[11px] font-medium leading-[16px] text-[#8d8d93]">
                {profileSubtitle}
              </p>
            </div>
          </div>
...
function InitialAvatar({
  ...
  return (
    <span
      aria-hidden="true"
      className={`flex shrink-0 items-center justify-center rounded-full border-2 border-[#0fc092] bg-[#15447c] font-bold text-white ${sizeClass}`}
    >
      {initial}
    </span>
  );
...
function Sidebar({
  ...
  return (
    <aside
      ...
      style={{
        background: "linear-gradient(270deg, #164780 0%, #001C42 100%)",
      }}
    >
      <a
        aria-label="AgroTrust dashboard"
        className="mb-6 block px-2"
        href="/dashboard"
      >
        <Image
          alt="AgroTrust"
          className="h-7 w-[139px] object-contain"
          height={28}
          priority
  ...
  return (
    <a
      ...
      className={`flex ${mobile ? "h-9 shrink-0 gap-2" : "h-[34px] gap-3"} items-center rounded-[5px] px-3 text-[12px] font-medium leading-[18px] tracking-[-0.15px] transition ${
        active
          ? "bg-[#15447C] text-white"
          : "text-[#b9c9dc] hover:bg-white/10 hover:text-white"
      }`}
      href={href}
      onClick={onClick}
    >
...
function SidebarIcon({ icon }: { icon: AppNavKey }) {
  switch (icon) {
    ...
    case "transactions":
      return <ReceiptText aria-hidden className="h-4 w-4 shrink-0" />;
    case "warehouses":
      return <Warehouse aria-hidden className="h-4 w-4 shrink-0" />;
    case "inventory":
      return <Boxes aria-hidden className="h-4 w-4 shrink-0" />;
    case "documents":
      return <FileText aria-hidden className="h-4 w-4 shrink-0" />;
    case "payments":
      return <WalletCards aria-hidden className="h-4 w-4 shrink-0" />;
    case "users":
      return <UserCircle aria-hidden className="h-4 w-4 shrink-0" />;
    case "analytics":
      return (
        <svg
          aria-hidden="true"
